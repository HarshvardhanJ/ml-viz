...
export function drawLossChart(svg, { x, y, w, h, history, color = "#ffb238", title = "Loss", maxPoints = 50 }) {
  ...
  const panelBg = cssVar("--ink-panel", "#14202f");
  ...
  g.appendChild(svgEl("rect", {
    ...
    fill: panelBg, stroke: borderColor, "stroke-width": 1,
  }));

  const titleEl = svgEl("text", { x: x + 8, y: y + 15, fill: dimText, "font-family": "IBM Plex Mono, monospace", "font-size": 10 });
  ...
  // baseline
  g.appendChild(svgEl("line", { x1: px0, y1: py0 + ph, x2: px0 + pw, y2: py0 + ph, stroke: borderColor, "stroke-width": 1 }));

  if (trimmed.length < 2) {
    const placeholder = svgEl("text", { x: px0 + pw / 2, y: py0 + ph / 2 + 4, "text-anchor": "middle", fill: dimText, "font-family": "IBM Plex Mono, monospace", "font-size": 10 });
